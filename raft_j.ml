...
let write_raft_persistent_state : _ -> raft_persistent_state -> _ =
 fun ob x ->
  ...
  Yojson.Safe.write_int ob x.current_term ;
  if !is_first then is_first := false else Bi_outbuf.add_char ob ',' ;
  Bi_outbuf.add_string ob "\"voted_for\":" ;
  Yojson.Safe.write_int ob x.voted_for ;
  if !is_first then is_first := false else Bi_outbuf.add_char ob ',' ;
...
let read_raft_persistent_state p lb =
  Yojson.Safe.read_space p lb ;
  Yojson.Safe.read_lcurl p lb ;
  ...
  try
    ...
    ( match i with
    | 0 ->
        field_current_term := Atdgen_runtime.Oj_run.read_int p lb ;
        bits0 := !bits0 lor 0x1
    | 1 ->
        field_voted_for := Atdgen_runtime.Oj_run.read_int p lb ;
        bits0 := !bits0 lor 0x2
    | 2 ->
  ...
  with Yojson.End_of_object ->
    if !bits0 <> 0x7 then
      Atdgen_runtime.Oj_run.missing_fields p [|!bits0|]
        [|"current_term"; "voted_for"; "entries"|] ;
...
let string_of_raft_mode ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_raft_mode ob x ; Bi_outbuf.contents ob

let read_raft_mode p lb =
  Yojson.Safe.read_space p lb ;
  ...
  | `Double_quote -> (
    ...
    | x -> Atdgen_runtime.Oj_run.invalid_variant_tag p x )
  | `Square_bracket -> (
    match Atdgen_runtime.Oj_run.read_string p lb with x ->
...
let write_raft_state : _ -> raft_state -> _ =
 fun ob x ->
  ...
  write_raft_mode ob x.mode ;
  if !is_first then is_first := false else Bi_outbuf.add_char ob ',' ;
  Bi_outbuf.add_string ob "\"per_state\":" ;
  write_raft_persistent_state ob x.per_state ;
  Bi_outbuf.add_char ob '}'

let string_of_raft_state ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_raft_state ob x ; Bi_outbuf.contents ob

let read_raft_state p lb =
  Yojson.Safe.read_space p lb ;
  Yojson.Safe.read_lcurl p lb ;
  ...
  try
    ...
    ( match i with
    | 0 ->
        field_mode := read_raft_mode p lb ;
        bits0 := !bits0 lor 0x1
    | 1 ->
        field_per_state := read_raft_persistent_state p lb ;
        bits0 := !bits0 lor 0x2
    | _ -> Yojson.Safe.skip_json p lb ) ;
    while true do
      Yojson.Safe.read_space p lb ;
  ...
  with Yojson.End_of_object ->
    if !bits0 <> 0x3 then
      Atdgen_runtime.Oj_run.missing_fields p [|!bits0|] [|"mode"; "per_state"|] ;
    ({mode= !field_mode; per_state= !field_per_state} : raft_state)

let raft_state_of_string s =
  read_raft_state (Yojson.Safe.init_lexer ()) (Lexing.from_string s)

let write_raft_event ob x =
  match x with
...
let string_of_raft_event ?(len = 1024) x =
  let ob = Bi_outbuf.create len in
  write_raft_event ob x ; Bi_outbuf.contents ob

let read_raft_event p lb =
  Yojson.Safe.read_space p lb ;
  match Yojson.Safe.start_any_variant p lb with
  | `Edgy_bracket -> (
    match Yojson.Safe.read_ident p lb with
